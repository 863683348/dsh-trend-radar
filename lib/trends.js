...
/** Repo-count series across snapshots, for the web panel's growth curve. */
export function computeSeries(snapshots) {
  return (snapshots ?? []).map((s) => ({ ts: s.ts, count: (s.repos ?? []).length }));
}

/**
 * Build the structured dashboard payload consumed by the web panel: growth
 * curve, category heat, and the top new/star-gainer lists. JSON-safe, capped
 * for the UI.
 * @param history - chronological snapshots.
 * @param report - a report computed over the same history.
 * @returns the panel payload.
 */
export function buildDashboard(history, report) {
  const latest = history[history.length - 1];
  return {
    ts: report.latestTs ?? latest?.ts ?? 0,
    snapshots: history.length,
    total: report.total,
    awesomeCoverage: report.awesomeCoverage,
    series: computeSeries(history).slice(-24),
    newPlugins: report.newPlugins.slice(0, 6),
    starGainers: report.starGainers.slice(0, 6),
    categoryHeat: report.categoryHeat.slice(0, 8),
  };
}
